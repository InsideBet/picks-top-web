...
# Traducciones ampliadas para cubrir todos los casos
TRADUCCIONES = {
    'Rk': 'POS', 'Squad': 'EQUIPO', 'MP': 'PJ', 'W': 'G', 'D': 'E', 'L': 'P',
    'GF': 'GF', 'GA': 'GC', 'GD': 'DG', 'Pts': 'PTS', 'PTS': 'PTS',
    'Last 5': 'ÚLTIMOS 5', 'Wk': 'JORNADA', 'Date': 'FECHA', 'Time': 'HORA',
    'Home': 'LOCAL', 'Away': 'VISITANTE', 'Venue': 'ESTADIO', 'Day': 'DÍA',
...
def cargar_excel(ruta_archivo, tipo="general"):
    ...
    try:
        ...
        # Limpieza de nombres de equipos común a todos
        for col in ['Squad', 'Home', 'Away', 'EQUIPO', 'LOCAL', 'VISITANTE']:
        ...
        if tipo == "stats":
            ...
            return df.rename(columns=TRADUCCIONES)
            ...
            # Eliminamos las columnas innecesarias del fixture original de FBRef
            drop_f = ['Attendance', 'Referee', 'Notes', 'Match Report']
            df = df.drop(columns=[c for c in drop_f if c in df.columns])
            # Reordenar y traducir
            df = df.rename(columns=TRADUCCIONES)
            # Seleccionamos solo las importantes para que no se vea sobrecargado
            cols_fix = ['JORNADA', 'FECHA', 'HORA', 'LOCAL', 'VISITANTE', 'ESTADIO']
            df = df[[c for c in cols_fix if c in df.columns]]
            return df

        return df.rename(columns=TRADUCCIONES).dropna(how='all')
    except:
        return None
...
<style>
    ...
    th { position: sticky; top: 0; background-color: #1f2937 !important; color: white; padding: 12px; border: 1px solid #374151; font-size: 13px; z-index: 10; }
    td { padding: 10px; border: 1px solid #374151; font-size: 14px; text-align: center !important; }
    div.stButton > button { background-color: #ff1800 !important; color: white !important; font-weight: bold !important; width: 100%; border-radius: 8px; border: none; }
    ...
    .bar-fill { background-color: #ff1800; height: 100%; }
    .bar-text { font-size: 12px; font-weight: bold; min-width: 32px; }
...
if st.session_state.menu_open:
    liga = st.selectbox("ELIGE UNA LIGA", ["---"] + LIGAS_ORDENADAS)
